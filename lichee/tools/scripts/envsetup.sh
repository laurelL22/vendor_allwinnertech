...
function chal()
{
    ...
    \cd $T/../chips/r528/drivers/rtos-hal
}
...
function get_all_projects()
{
    ...
    for f in `ls -l ${T}/../boards/r528/ | awk '/^d/{print $NF}'`;
    do
		if [ "${f}" == "drivers"  ]; then
			continue;
		fi
        ...
        for n in `ls -l ${T}/../boards/r528/${f}/configs/ | awk '/^d/{print $NF}'`;
        do
            project_name=${f}_${n}
    ...
    for f in `ls -l ${T}/../boards/r528/ | awk '/^l/{print $(NF-2)}'`;
    do
        exit_flag=0;
        local project_name=${f};
        for n in `ls -l ${T}/../boards/r528/${f}/configs/ | awk '/^d/{print $NF}'`;
...
function lunch_nuttx()
{
	...
	export RTOS_TARGET_ARCH=arm
	export RTOS_TARGET_CHIP=sun8iw20p1

	export RTOS_TARGET_PROJECT_PATH=${RTOS_TARGET_DEVICE/-//}_nand
	export RTOS_TARGET_BOARD_PATH=${RTOS_TARGET_DEVICE/-//}_nand
	export RTOS_PROJECT_CONFIG_PATH=${select#${RTOS_TARGET_DEVICE}_}
	export RTOS_CONFIG_PATH=${T}/../boards/r528/${RTOS_TARGET_DEVICE}/configs/${RTOS_PROJECT_CONFIG_PATH}
	...
	#	cp ${NUTTXDIR}/.config ${ROOTDIR}/nuttx/boards/*/*/${1/[:|\/]//configs/}/defconfig
	#else
	#	cp ${NUTTXDIR}/.config ${RTOS_CONFIG_PATH}/defconfig
	#fi
}
...
{
    local T="$(gettop)"
	local AP_CONFIG_PATH=${T}/../boards/r528/${RTOS_TARGET_DEVICE}/configs/ap
	local OTA_CONFIG_PATH=${T}/../boards/r528/${RTOS_TARGET_DEVICE}/configs/ota
	local BL_CONFIG_PATH=${T}/../boards/r528/${RTOS_TARGET_DEVICE}/configs/bootloader
...
function mnuttx()
{
    ...
    #if [ ! -d ${RTOS_CONFIG_PATH} ]; then
    #    cp ${ROOTDIR}/nuttx/boards/*/*/${1/[:|\/]//configs/d}/defconfig ${NUTTXDIR}/.config
    #else
    #    cp  ${RTOS_CONFIG_PATH}/defconfig ${NUTTXDIR}/.config
    #fi
    ...
    if [ 1 -eq 1 ]; then
        ...
        (cd ${T}/../../../ && ./build.sh vendor/allwinnertech/boards/r528/${RTOS_TARGET_DEVICE}/configs/${RTOS_PROJECT_CONFIG_PATH} -j${JOBS} $@ )
        [ $? -ne 0 ] \
            && echo "**********make nuttx fail***********" \
            && return 1
    else
		(cd ${T}/../../../ && ./build.sh vendor/allwinnertech/boards/r528/${RTOS_TARGET_DEVICE}/configs/${RTOS_PROJECT_CONFIG_PATH} $@)
        [ $? -ne 0 ] \
            && echo "**********make nuttx fail***********" \
            && return 1
    fi
...
function mlib()
{
    ...
    if [ "${RTOS_TARGET_ARCH}" == "arm"  ];then
		/data/openvela0729/prebuilts/gcc/linux-x86_64/arm-none-eabi/bin/arm-none-eabi-ar rv -o $2 ${ALL_OBJECTS_FILES}
