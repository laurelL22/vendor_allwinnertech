...
if [ "x${PACK_BOARD}" == "xr528s3-x4b" ] ; then
LOGICAL_PARTS_KB_FOR_256Mnand=$(( 245120 - 256 ))
else
...
board/${PACK_PROJECT_PATH}/bin/recovery.fex:image/recovery.fex
...
function mklittlefs()
{
	...
	if [ "x${PACK_HOSTOS}" == "xlinux" ] ; then
		# read_size and program_size set to 4096 for xiaomi-vela
		${MKLFS} -c $2 -b $((4 * 1024)) -r 4096 -p 4096 -s $1 -i $3
		if [ $? -ne 0 ]; then
...
function mkyaffs()
{
	echo "--- create yaffs2 image ---"
	# Only Linux platform
	mkyaffs2image $1 $2 $3
	echo "--- create yaffs2 image end ---"
}

function mkromfs()
{
	echo "--- create romfs image ---"
	#Only Linux platform
	genromfs -f $2 -d $1 -V $3 -a $4
	echo "--- create romfs image end ---"
}
...
function make_data_image()
{
	...
	do
		...
		size=$(( 2 * 1024 * 1024 ))
		echo "mklittlefs ${size} ${name} ${downloadfile}"
		mklittlefs ${size} ${name} ${downloadfile}
		#if have user_data file,update user_data size info to partition config file
		if [ $? -eq 0 ] ; then
			if [ "x${part_name}" == "xuser_data" ] ; then
				eval ${3}=${part_size}
			fi
		fi
...
function make_res_image()
{
	...
	do
		...
		size="$(awk -F: '{print $2}' <<< "${part}")"
		downloadfile="$(awk -F: '{print $3}' <<< "${part}" | sed 's/"//g')"
		grep "^res" <<< "${downloadfile}" || continue
		...
		echo "copy ap.fex to ${dir_name}/ap"
		mkdir -p ${dir_name}/ap
		cp -rf ${PACK_TOPDIR}/board/${PACK_PROJECT_PATH}/configs/ap.fex ${dir_name}/ap/
		name="${dir_name}"
		echo "The filesystem path: ${name}"

		downloadfile="${ROOT_DIR}/${PACK_PROJECT_PATH}/image/${downloadfile}"
		#mkspiffs ${size} ${name} ${downloadfile}
		echo "mkromfs ${name} ${downloadfile}"
		mkromfs ${name} ${downloadfile} "X4BResource" 2048
		local tr_size
		tr_size=`stat -c %s ${downloadfile}`
		echo ori_size=${tr_size}
		tr_size=$((${tr_size} + 4096))
		tr_size=$((${tr_size} & ~4095))
		echo tr_size=${tr_size}
		truncate -s ${tr_size} ${downloadfile}

	done
}

function make_usrdata_image()
{
	local part name size downloadfile dir_name
	...
	do
		...
		size="$(awk -F: '{print $2}' <<< "${part}")"
		downloadfile="$(awk -F: '{print $3}' <<< "${part}" | sed 's/"//g')"
		grep "^usrdata" <<< "${downloadfile}" || continue
		...
		if [ ! -d ${dir_name} ]; then
			pack_error "not found ${dir_name} for creating ${downloadfile}, try default data!!!!!"
			dir_name="${PACK_TOPDIR}/board/common/data/UDISK"
			if [ ! -d ${dir_name} ]; then
				pack_error "not found ${dir_name} for creating ${downloadfile}"
				continue
			fi
		...
		downloadfile="${ROOT_DIR}/${PACK_PROJECT_PATH}/image/${downloadfile}"
		#mkspiffs ${size} ${name} ${downloadfile}
		echo "mkyaffs ${name} ${downloadfile} 2048"
		mkyaffs ${name} ${downloadfile} 2048
		local tr_size
		tr_size=`stat -c %s ${downloadfile}`
		echo ori_size=${tr_size}
		tr_size=$((${tr_size} + 4096))
...
function do_prepare()
{
	...
	if [ "x${PACK_CHIP}" = "xsun20iw2p1" ]; then
		rtos_config=${PACK_TOPDIR}/lichee/rtos/projects/${PACK_PROJECT_PATH}_m33/defconfig
		if grep -q "CONFIG_XIP=y" ${PACK_TOPDIR}/lichee/rtos/projects/${PACK_PROJECT_PATH}_m33/defconfig; then
			config_xip=1
		fi
		if grep -q "CONFIG_XIP=y" ${PACK_TOPDIR}/lichee/rtos/projects/${PACK_PROJECT_PATH}_c906/defconfig; then
			config_xip=1
		fi
	else
		rtos_config=${PACK_TOPDIR}/lichee/rtos/projects/${PACK_PROJECT_PATH}/defconfig
	fi
	...
	[ -f ${ROOT_DIR}/${PACK_PROJECT_PATH}/image/bl.fex ] && {
		gzip -c ${ROOT_DIR}/${PACK_PROJECT_PATH}/image/bl.fex > ${ROOT_DIR}/${PACK_PROJECT_PATH}/image/freertos-gz.fex
		if [ "x${PACK_HOSTOS}" == "xlinux" ] ; then
			${LZ4} ${ROOT_DIR}/${PACK_PROJECT_PATH}/image/bl.fex ${ROOT_DIR}/${PACK_PROJECT_PATH}/image/freertos-lz4.fex
		elif [ "x${PACK_HOSTOS}" == "xwin" ] ; then
	...
	[ -f ${ROOT_DIR}/${PACK_PROJECT_PATH}/image/recovery.fex ] && {
		gzip -c ${ROOT_DIR}/${PACK_PROJECT_PATH}/image/recovery.fex > ${ROOT_DIR}/${PACK_PROJECT_PATH}/image/recovery-gz.fex
		if [ "x${PACK_HOSTOS}" == "xlinux" ] ; then
			${LZ4} ${ROOT_DIR}/${PACK_PROJECT_PATH}/image/recovery.fex ${ROOT_DIR}/${PACK_PROJECT_PATH}/image/recovery-lz4.fex
		elif [ "x${PACK_HOSTOS}" == "xwin" ] ; then
			local recovery_fex=$(cygpath -w ${ROOT_DIR}/${PACK_PROJECT_PATH}/image/recovery.fex)
			local recovery_lz4_fex=$(cygpath -w ${ROOT_DIR}/${PACK_PROJECT_PATH}/image/recovery-lz4.fex)
			${LZ4} ${recovery_fex/\\/\\\\} ${recovery_lz4_fex/\\/\\\\}
		fi
	}
	...
	if [ "x${config_xip}" = "x1" ] ; then
		if [ "x${PACK_SECURE}" = "xsecure"  -o "x${PACK_SIG}" = "xsecure" ] ; then
			...
			cp -vf ${ROOT_DIR}/${PACK_PROJECT_PATH}/image/image_header_secure_xip.cfg ${ROOT_DIR}/${PACK_PROJECT_PATH}/image/image_header_secure.cfg
		else
			echo "CONFIG_XIP=y, use sys_partition_xip.fex"
			cp -vf ${ROOT_DIR}/${PACK_PROJECT_PATH}/image/sys_partition_xip.fex ${ROOT_DIR}/${PACK_PROJECT_PATH}/image/sys_partition_nor.fex
			cp -vf ${ROOT_DIR}/${PACK_PROJECT_PATH}/image/image_header_xip.cfg ${ROOT_DIR}/${PACK_PROJECT_PATH}/image/image_header.cfg
		fi
	fi
	...
	if [ "x${PACK_SECURE}" != "xnone" -o "x${PACK_SIG}" != "xnone" ]; then
		...
		elif [ -f "${ROOT_DIR}/${PACK_PROJECT_PATH}/image/image_header_secure.cfg" ]; then
			MAIN_VERION=`grep '"pversion"' ${ROOT_DIR}/${PACK_PROJECT_PATH}/image/image_header_secure.cfg | awk -F \" '{print $4}'`
		fi
...
function do_common()
{
	...
	${UPDATE_BOOT0} boot0_sdcard.fex	sys_config${SUFFIX}.bin SDMMC_CARD 576
...
function do_finish()
{
	if [ -f sys_partition_nor.bin ]; then
		...
		if [ ${current_rtos_full_img_size} -eq 4 ]; then
			...
			pack_error "full img size ${current_rtos_full_img_size} is not 4/8/16M"
			exit 1;
		fi
...
function do_signature_ih()
{
	...
	grep "key_type" ${ROOT_DIR}/${PACK_PROJECT_PATH}/image/image_header_secure.cfg
	if [ $? -eq 0 ]
	then
		key_type=`grep '"key_type"' ${ROOT_DIR}/${PACK_PROJECT_PATH}/image/image_header_secure.cfg | awk -F \" '{print $4}'`
	else
		key_type=rsa
	fi
	if [ "$key_type" == "rsa" ]
	...
	${MKIMAGE_HEADER} -C 5 -e flash_crypto.ini -c image_header_secure.cfg -s $key_type -k ${ROOT_DIR}/${PACK_PROJECT_PATH}/keys
...
function do_pack_rtos()
{
	...
	if [ "x${PACK_SIG}" = "xsecure" ] ; then
		...
		if [ "x${storage_type}" = "x5" ] ; then
			${UPDATE_RTOS} --image bl.fex \
				--cert toc1/cert/bl.der \
				--output bl-update.fex
			if [ $? -ne 0 ]; then
				pack_error "add rtos header error!"
				exit 1
			fi
			mv bl-update.fex bl.fex
			#ln -s freertos-gz-update.fex rtos_pkg_nor.fex
			#ln -s freertos-gz-update.fex rtos_pkg.fex

			##for ota image
			#[ -f recovery-gz.fex ] && {
			#	${UPDATE_RTOS} --image recovery-gz.fex \
			#		--cert toc1/cert/recovery-gz.der \
			#		--output recovery-gz-update.fex
			#	if [ $? -ne 0 ]; then
			#		pack_error "add rtos header error!"
			#		exit 1
			#	fi
			#	ln -s recovery-gz-update.fex rtos_ota_nor.fex
			#	ln -s recovery-gz-update.fex rtos_ota.fex
			#}
		fi
		if [ "x${storage_type}" = "x3" ] ; then
		...
		echo "normal"
		if [ "x${storage_type}" = "x5" ] ; then
			echo "nuttx not use freertos.fex"
			#${UPDATE_RTOS} --image freertos-gz.fex \
			#	--output freertos-gz-update.fex
			#if [ $? -ne 0 ]; then
			#	pack_error "add rtos header error!"
		...
		if [ "x${storage_type}" = "x3" ] ; then
			...
			if grep -q "CONFIG_XRADIO_HEADER=y" $rtos_config; then
				if [ "x${PACK_HOSTOS}" == "xwin" ] ; then
					cp ${PACK_TOPDIR}/tools/win-tools/msys-2.0.dll ${ROOT_DIR}/${PACK_PROJECT_PATH}/image
				fi
				...
				busybox unix2dos ih_boot_package_nor.cfg
				${DRAGONSECBOOT} -pack ih_boot_package_nor.cfg
				if [ $? -ne 0 ]
				then
					pack_error "dragon pack nor run error"
					exit 1
...
function create_rtos_full_img()
{
		...
		if grep -q "CONFIG_ARCH_SUN20IW2P1=y" $rtos_config; then
			LOGIC_START=128
		else
			LOGIC_START=112
		fi
...
function prepare_for_128Mnand()
{
	...
	make_res_image sys_partition.fex ${LOGICAL_PARTS_KB_FOR_128Mnand} "cut_zero"
	make_usrdata_image sys_partition.fex ${LOGICAL_PARTS_KB_FOR_128Mnand} "cut_zero"
...
function prepare_for_256Mnand()
{
	...
	make_res_image sys_partition.fex ${LOGICAL_PARTS_KB_FOR_256Mnand} "cut_zero"
	make_usrdata_image sys_partition.fex ${LOGICAL_PARTS_KB_FOR_256Mnand} "cut_zero"
...
if [ "x${PACK_MODE}" = "xdump" ] ; then
	...
	if grep -q "CONFIG_PROJECT_R128S1=y" $rtos_config; then
	    prepare_for_8Mnor
	    do_finish
	else
	    prepare_for_16Mnor
