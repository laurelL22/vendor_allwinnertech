...
static int setSoundDeviceParams(SoundCtrlContext* sc)
{
    ...
    TLOGD("sc->alsa_fragcount = %d, sc->chunk_size = %d\n", sc->alsa_fragcount, sc->chunk_size);
    ...
    if ((ret = snd_pcm_hw_params_set_format(sc->alsa_handler, sc->alsa_hwparams,
                sc->alsa_format)) < 0)
    {
        TLOGE("nd_pcm_hw_params_set_channelsnd_pcm_hw_params_set_format failed:%s",strerror(errno));
        goto SET_PAR_ERR;
    }
    
    if ((ret = snd_pcm_hw_params_set_channels(sc->alsa_handler,
            sc->alsa_hwparams, sc->nChannelNum)) < 0) {
        TLOGE("snd_pcm_hw_params_set_channels failed:%s, ch_num=%d",strerror(errno), sc->nChannelNum);
        goto SET_PAR_ERR;
    }
...
void TSoundDeviceDestroy(SoundCtrl* s){
    ...
    if(PostProcPar.InputPcmBuf)
    {
        free(PostProcPar.InputPcmBuf);
        PostProcPar.InputPcmBuf = NULL;
    }
    if(PostProcPar.OutputPcmBuf)
    {
        free(PostProcPar.OutputPcmBuf);
        PostProcPar.OutputPcmBuf = NULL;
    }
    memset(&PostProcPar, 0, sizeof(PostProcessSt));
}
...
int TSoundDeviceWrite(SoundCtrl* s, void* pData, int nDataSize){
    ...
    int gainRet = do_AudioGain(&audioGain);
    if(gainRet == 0){
	...
	//TLOGD("snd_pcm_writei begin,nDataSize = %d, num_frames=%d",nDataSize, num_frames);
