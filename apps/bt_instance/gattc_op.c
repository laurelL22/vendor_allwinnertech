...
//int gattc_create_cmd(void* handle, int argc, char* argv[]);
static int delete_cmd(void* handle, int argc, char* argv[]);
//int gattc_connect_cmd(void* handle, int argc, char* argv[]);
//int disconnect_cmd(void* handle, int argc, char* argv[]);
...
//int gattc_write_cmd(void* handle, int argc, char* argv[]);
...
//set flag by other application
static atomic_bool g_need_send = ATOMIC_VAR_INIT(false);
static atomic_int  g_brightness = ATOMIC_VAR_INIT(0);
static pthread_t g_tid;
static bool g_conn_success = false;
...
static bt_command_t g_gattc_tables[] = {
    { "create", gattc_create_cmd, 0, "\"create gatt client :\"" },
    { "delete", delete_cmd, 0, "\"delete gatt client :<conn id>\"" },
    { "connect", gattc_connect_cmd, 0, "\"connect remote device :<conn id><address>[addr type(0:public,1:random,2:public_id,3:random_id)]\"" },
    { "disconnect", gattc_disconnect_cmd, 0, "\"disconnect remote device :<conn id>\"" },
    ...
    { "write_cmd", gattc_write_cmd, 0, "\"write cmd :<conn id><char id><type>(str or hex)<playload>\n"
...
int gattc_connect_cmd(void* handle, int argc, char* argv[])
{
    ble_addr_type_t addr_type = BT_LE_ADDR_TYPE_PUBLIC;
    if (argc < 2)
...
int gattc_disconnect_cmd(void* handle, int argc, char* argv[])
{
    if (argc < 1)
        return CMD_PARAM_NOT_ENOUGH;
...
{
    int i = 0;
    while (g_conn_success)
    {
        if (atomic_exchange(&g_need_send, false))
        {
            uint8_t val = (uint8_t)atomic_load(&g_brightness);
            float perc = val * 1.0f / 100;
            do_light_ctrl(perc);
        }
        i++;
        sleep(2); 
...
static void connect_callback(void* conn_handle, bt_address_t* addr)
{
    ...
    //loop light control from apps 
    pthread_attr_t attr;
    pthread_attr_init(&attr);
    pthread_attr_setstacksize(&attr, 12288);
    pthread_create(&g_tid, &attr, light_thread, NULL);
...
int gattc_command_exec(void* handle, int argc, char* argv[])
{
    ...
    if (argc > 0)
        ret = execute_cmd_in_table(handle, g_gattc_tables, ARRAY_SIZE(g_gattc_tables), argc, argv);
...
void light_request_set(uint8_t level)
{
    atomic_store(&g_brightness, level);
    atomic_store(&g_need_send, true);
}
